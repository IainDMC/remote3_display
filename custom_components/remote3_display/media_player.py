...
async def async_setup_platform(
    ...
    """Set up the Remote 3 display media player from YAML."""
    _LOGGER.warning("Setting up remote3_display media player: %s", config[CONF_NAME])
    entity = Remote3DisplayMediaPlayer(hass, config)
...
class Remote3DisplayMediaPlayer(MediaPlayerEntity):
    ...
    def __init__(self, hass: HomeAssistant, config: dict) -> None:
        ...
        self._app_logos = config[CONF_APP_LOGOS]
        self._app_names = config[CONF_APP_NAMES]
        self._fallback_logo = config[CONF_FALLBACK_LOGO]
        self._tmdb_token = config.get(CONF_TMDB_TOKEN)
        self._tmdb_enabled = config[CONF_TMDB_ENABLED]
        self._tmdb_language = config[CONF_TMDB_LANGUAGE]
        self._tmdb_cache: dict[str, str | None] = {}
        self._tmdb_pending: set[str] = set()
        self._tivimate_enabled = config[CONF_TIVIMATE_ENABLED]
        self._tivimate_app_id = config[CONF_TIVIMATE_APP_ID]
        self._tivimate_adb_entity = config.get(CONF_TIVIMATE_ADB_ENTITY)
        self._tivimate_channel_resource_id = config[CONF_TIVIMATE_CHANNEL_RESOURCE_ID]
        self._tivimate_program_resource_id = config[CONF_TIVIMATE_PROGRAM_RESOURCE_ID]
        self._tivimate_time_resource_id = config[CONF_TIVIMATE_TIME_RESOURCE_ID]
        self._tivimate_poll_seconds = config[CONF_TIVIMATE_POLL_SECONDS]
        self._tivimate_mode = config[CONF_TIVIMATE_MODE]
        self._tivimate_webhook_id = config.get(CONF_TIVIMATE_WEBHOOK_ID)
        self._tivimate_retain_last = config[CONF_TIVIMATE_RETAIN_LAST]
        ...
        self._xtream_enabled = config[CONF_XTREAM_ENABLED]
        ...
        self._tivimate_artwork = config[CONF_TIVIMATE_ARTWORK]
        self._tivimate_channel_icon_scale = config[CONF_TIVIMATE_CHANNEL_ICON_SCALE]
        self._icon_entity_key = slugify(self._attr_name)
        self._icon_access_token = secrets.token_urlsafe(24)
        self._scaled_icon_cache: dict[tuple[str, int], tuple[bytes, str]] = {}
        self._xtream_icons: dict[str, str] = {}
        self._xtream_pending = False
        self._xtream_last_attempt: datetime | None = None
        self._xtream_error = ""
        self._playlist_enabled = config[CONF_PLAYLIST_ENABLED]
        self._playlist_url = config.get(CONF_PLAYLIST_URL)
        self._playlist_urls = list(config[CONF_PLAYLIST_URLS])
        ...
        self._xmltv_programs: dict[str, list[tuple[datetime, datetime, str]]] = {}
    ...
    def _schedule_tivimate_poll(self) -> None:
        ...
        if self.app_id != self._tivimate_app_id:
            if not self._tivimate_retain_last and (
            ...
            self._tivimate_status = (
                ...
                if self._tivimate_retain_last and self._tivimate_channel
                else "inactive"
            )
            self._tivimate_error = ""
    ...
    def _schedule_playlist_refresh(self) -> None:
        ...
        if (
            ...
            and now - self._playlist_last_attempt < XTREAM_REFRESH
        ):
            return
    ...
    async def _async_fetch_playlist_icons(self) -> None:
        ...
        try:
            icons: dict[str, str] = {}
            programs: dict[str, list[tuple[datetime, datetime, str]]] = {}
            errors = []
            ...
            for index, source_url in enumerate(self._playlist_urls, start=1):
                try:
                    ...
                    if "#EXTM3U" in playlist[:1000].upper():
                        source_icons = self._parse_m3u_icons(playlist, final_url)
                        source_programs = {}
    ...
    def _parse_xmltv_data(
        ...
        dict[str, str], dict[str, list[tuple[datetime, datetime, str]]]
    ]:
    ...
    def _normalize_channel(value) -> str:
        ...
        return Remote3DisplayMediaPlayer._clean_channel_name(value).casefold()

    @staticmethod
    def _channel_match_tokens(value) -> tuple[str, ...]:
        """Return stable words used by the conservative logo fallback matcher."""
        normalized = Remote3DisplayMediaPlayer._normalize_channel(value)
        return tuple(re.findall(r"[a-z0-9]+", normalized, flags=re.IGNORECASE))

    @staticmethod
    def _clean_channel_name(value) -> str:
    ...
    async def async_get_scaled_channel_icon(self) -> tuple[bytes, str] | None:
        ...
        if not icon_url:
            return None
        cache_key = (icon_url, self._tivimate_channel_icon_scale)
    ...
    @staticmethod
    def _scale_icon_bytes(original: bytes, scale_percent: int) -> bytes:
        ...
        canvas.alpha_composite(
            resized, ((width - target[0]) // 2, (height - target[1]) // 2)
        )
        output = BytesIO()
    ...
    def _update_program_from_xmltv(self, now: datetime) -> bool:
        ...
        if self._tivimate_end is not None and now < self._tivimate_end:
            return False
    ...
    def media_image_url(self):
        ...
        if self._media_active():
            ...
            if (
                self.app_id == self._tivimate_app_id
                and self._tivimate_artwork == "channel_icon"
                and channel_icon
            ):
                return (
                    self._scaled_channel_icon_url(channel_icon)
                    if self._tivimate_channel_icon_scale < 100
                    else channel_icon
                )
            tmdb_image = self._tmdb_cache.get(self.media_title)
            if tmdb_image:
                return tmdb_image
            if self.app_id == self._tivimate_app_id and channel_icon:
                return (
                    self._scaled_channel_icon_url(channel_icon)
                    if self._tivimate_channel_icon_scale < 100
                    else channel_icon
    ...
    def extra_state_attributes(self):
        return {
            ...
            "xmltv_history_hours": int(XMLTV_HISTORY.total_seconds() / 3600),
            "xmltv_future_hours": int(XMLTV_FUTURE.total_seconds() / 3600),
